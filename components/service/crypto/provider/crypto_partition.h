...
/**
 * Concerned with partitioning of the crypto service backed key store
 * to protect keys and key store resource.  Key partitioning for
 * stored keys is handled by associating keys with a namespace.
 */
 ...
 */
key_id_namespace_t crypto_partition_get_namespace(
	uint32_t client_id);
 ...
 */
namespaced_key_id_t crypto_partition_get_namespaced_key_id(
	uint32_t client_id,
	psa_key_id_t key_id);

/**
 *  \brief Associate a key with a partition
 *
 *	\param attributes	Key attributes object
 * 	\param client_id	The uniform identifier for the client
 */
void crypto_partition_associate(
