...
TEST_GROUP(CryptoSecurityPartitioningTests)
{
	void setup()
	{
		...
		/* Open two RPC sessions, one for client a and the other for client b.  Both have different
		 * client identities, assigned when the rpc sessiion is opened.
		 */
		m_rpc_session_a = service_context_open(m_crypto_service_context, TS_RPC_ENCODING_PACKED_C, &caller);
		CHECK_TRUE(m_rpc_session_a);
		m_client_a = new packedc_crypto_client(caller);

		m_rpc_session_b = service_context_open(m_crypto_service_context, TS_RPC_ENCODING_PACKED_C, &caller);
		CHECK_TRUE(m_rpc_session_b);
		m_client_b = new packedc_crypto_client(caller);
	}
	...
	rpc_session_handle m_rpc_session_a;
	rpc_session_handle m_rpc_session_b;
